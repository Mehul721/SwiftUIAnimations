...
struct VCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading,spacing: 8){
            Text("Animations in SwiftUI app for iOS 16")
                .customFont(.title2)
                .frame(maxWidth: 170, alignment: .leading)
                .layoutPriority(1)//gives preference to this text
            ...
            Spacer()
            HStack{
                ForEach(Array([4,5,6].shuffled().enumerated()),
                id: \.offset){
                    index,number in
                    Image("Avatar \(number)")
                        .resizable()
        ...
        .shadow(color:course.color.opacity(0.3),radius: 2,x: 0,y: 12)
